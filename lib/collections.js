...
// NOTE: This is an alternative to the below way:
//    uniqueWords = Object.keys(uniqueWordsAsKeys);

// if you put this inside the for-loop, it will reset to 0
// every time, and then it won't work!
let count = 0;
for (let key in uniqueWordsAsKeys) {
    uniqueWords[count] = key;
    count++;
    // you can also replace 'count' with uniqueWords.length since
    // it will increase by one each time we add something. It's
    // clever, but maybe a bit hard to read.
}
...
//NOTE: THis is the way I originally did it. 
// loop through the keys in the uniqueWordsAsKeys Object, and for each key,
// compare that key with the words in NormalizedWords. If they match, increase
// the count by one, and set the count as the value for that key in wordFrequencies
for (let key in uniqueWordsAsKeys) {
  let count = 0;
  for (let i = 0; i < normalizedWords.length; i++) {
